...
import { Clock, MapPin, Users, CheckCircle2, AlertCircle, ArrowLeft, MessageSquare, Star } from 'lucide-react';
...
export default function ShopDetailsPage() {
  ...
  useEffect(() => {
    const fetchInfos = async () => {
        if (!id) return;
        console.log(`[DEBUG] Fetching details for Shop ID: ${id} (Number: ${Number(id)})`);
        setLoading(true);
        try {
            ...
            try {
                ...
                setQueue(qRes.data);
            } catch(e) { console.error("[DEBUG] Queue fetch failed", e); }
            
            try {
                const rRes = await shopApi.getReviews(Number(id));
                setReviews(rRes.data);
            } catch(e) { console.error("[DEBUG] Reviews fetch failed", e); }
  ...
  useEffect(() => {
    if (bookingMode === 'appointment' && selectedDate && id) {
        shopApi.getSlots(Number(id), selectedDate)
  ...
  const handleJoinQueue = async () => {
    ...
    try {
      ...
      toast.success(bookingMode === 'appointment' ? "Appointment Booked!" : "Joined Queue!");
      ...
      setSelectedSlot(null);
       ...
       console.error(error);
       const msg = error.response?.data || error.message || "Failed to process request.";
       // If data is object with message property
       const finalMsg = typeof msg === 'object' && msg.message ? msg.message : (typeof msg === 'string' ? msg : "Failed to process request.");
  ...
  if (loading) return <div className="text-center py-20 text-muted-foreground animate-pulse">Loading shop details...</div>;
  if (!shop) return <div className="text-center py-20 text-destructive">Shop not found</div>;

  const currentWaitTime = queue.reduce((acc, entry) => {
    if (entry.status === 'in_chair') return acc + (entry.service?.durationMins || 20) / 2;
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="mb-8">
           ...
           <h1 className="text-4xl font-extrabold tracking-tight mb-2">{shop.name}</h1>
        ...
        <div className="lg:col-span-2 space-y-6">
            <Card className="overflow-hidden shadow-md">
                <CardHeader className="bg-muted/30 pb-4">
                    ...
                    <CardDescription>Choose a service to join the queue.</CardDescription>
                </CardHeader>
                                ...
                                return (
                                    ...
                                    >
                                        <div className="flex items-center gap-4">
                                            {/* Custom Radio Button */}
                                            <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition-colors ${isSelected ? 'border-primary' : 'border-muted-foreground/40 bg-background'}`}>
                                                {isSelected && <div className="w-2.5 h-2.5 bg-primary rounded-full shadow-sm" />}
                                            </div>
                                    ...
                                    {slots.map((slot) => (
                                        <button
                                            ...
                                            className={`py-2 px-1 text-sm rounded border transition-colors
                                                ...
                                                selectedSlot === slot.time ? 'bg-primary text-primary-foreground border-primary' : 'hover:bg-accent border-input'}
                                            `}
                                        >
                                            {slot.time}
                                        </button>
                                    ))}
                                    {slots.length === 0 && <p className="text-sm text-muted-foreground col-span-3">No slots available for this date.</p>}
                                </div>
                            </div>
                        </div>
                    )}
                    ...
                    <div className="space-y-4 pt-4 border-t">
                        ...
                        >
                            {!selectedService 
                                ...
                                : (bookingMode === 'appointment' ? 'Book Appointment' : 'Join Queue Now')}
                        </Button>
            ...
            <Card className="border-t-4 border-t-primary shadow-xl">
                <CardHeader className="pb-2">
                    <div className="flex justify-between items-center">
                        ...
                        <Badge variant="destructive" className="animate-pulse px-2 py-0.5">LIVE</Badge>
                    </div>
                ...
                <CardContent>
                     <div className="py-6 text-center bg-muted/30 rounded-lg mb-6 border border-dashed">
                        <div className="text-sm font-medium text-muted-foreground">Est. Wait Time</div>
                        <div className="text-4xl font-black text-foreground my-1">{Math.ceil(currentWaitTime)} <span className="text-base font-normal text-muted-foreground">mins</span></div>
                    ...
                    <div className="space-y-4 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
                        {queue.length === 0 ? (
                            ...
                            queue.map((entry, index) => (
                                <div key={entry.id} className="relative pl-6 pb-6 last:pb-0 border-l border-muted-foreground/20">
                                    <div className={`absolute -left-[9px] top-0 w-4 h-4 rounded-full border-2 ${index === 0 ? 'bg-primary border-primary' : 'bg-background border-muted-foreground/40'}`} />
                                    
                                    <div className="bg-card p-3 rounded-lg border shadow-sm -mt-2">
                                        <div className="flex justify-between items-start mb-1">
                                            <div className="font-semibold text-sm">{entry.customerName}</div>
                                             {entry.status === 'in_chair' && (
                                                <Badge variant="secondary" className="bg-green-100 text-green-700 hover:bg-green-200 border-0 h-5">In Chair</Badge>
                                            )}
                                            {entry.appointmentTime && (
                                                <Badge variant="outline" className="text-[10px] border-amber-500 text-amber-600 bg-amber-50 h-5">Scheduled</Badge>
                                            )}
                                        </div>
                                        <div className="text-xs text-muted-foreground flex justify-between items-center">
                                            ...
                                            <span className="font-medium whitespace-nowrap ml-2">
                                                {entry.appointmentTime 
                                                    ? <>{new Date(entry.appointmentTime).toLocaleDateString(undefined, {month:'short', day:'numeric'})} {new Date(entry.appointmentTime).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</>
            ...
            <Card className="bg-blue-600 text-white border-none">
                <CardContent className="p-6">
                    ...
                    <p className="text-sm opacity-90">
                        Please arrive at least 10 minutes before your estimated time. Late arrivals may lose their spot.
                    </p>
                </CardContent>
            </Card>
        </div>
              ...
              <CardHeader>
                  <CardTitle className="flex justify-between items-center">
                      ...
                      <div className="flex items-center gap-2 bg-muted/50 px-3 py-1 rounded-full">
                          <span className="text-2xl font-bold">{shop.rating?.toFixed(1) || '0.0'}</span>
                          <RatingStars rating={shop.rating || 0} size={20} />
                                          ...
                                          <div className="flex gap-2">
                                              {[1, 2, 3, 4, 5].map((star) => (
                                                  ...
                                                  >
                                                     <Star 
                                                          size={32} 
                                                          className={star <= newReview.rating ? "fill-yellow-400 text-yellow-400" : "text-muted/30"} 
                                                      />
                                                  </button>
                                              ))}
                                          </div>
